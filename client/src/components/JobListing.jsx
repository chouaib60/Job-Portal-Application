    ...
    <div className='container 2xl:px-20 mx-auto flex flex_col lg:flex-row max-lg:space-y-8 py-8'>
      ...
      <div className='w-full lg:w-1/4 bg-white px-4'>
        ...
        {isSearched && (searchFilter.title !== "" || searchFilter.location !== "") && (
          <>
            ...
            <div className='mb-4 text-gray-600'>
              {searchFilter.title && (
                <span 
                  key="title-filter" // ⬅️ AJOUT DE KEY
              ...
              {searchFilter.location && (
                <span 
                  key="location-filter" // ⬅️ AJOUT DE KEY
        ...
        <div className={showFilter ? "" : "max-lg:hidden"}>
          ...
          <ul className='space-y-4 text-gray-600'>
            {JobCategories.map((category, index) => (
              <li 
                key={`category-${index}`} // ⬅️ AJOUT DE KEY
        ...
        <div className={showFilter ? "" : "max-lg:hidden"}>
          ...
          <ul className='space-y-4 text-gray-600'>
            {JobLocations.map((location, index) => (
              <li 
                key={`location-${index}`} // ⬅️ AJOUT DE KEY
                className='flex gap-3 items-center'
              >
                <input className='scale-125' type="checkbox" />
                {location}
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4'>
          {jobs.map((job) => (
            <JobCard 
              key={job.id} // ⬅️ KEY BASÉE SUR L'ID DU JOB (déjà présent)
              job={job} 
            />
          ))}
        </div>
        ...
        {jobs.length > 0 && (
          <div className='flex items-center justify-center space-x-2 mt-10'>
            <a href="#job-list">
              <img src={left_arrow_icon} alt="Previous page" />
            </a>
            
            {Array.from({ length: Math.ceil(jobs.length / 6) }).map((_, index) => (
              <a 
                key={`page-${index}`} // ⬅️ AJOUT DE KEY
                href="#job-list"
              >
                <button 
                  className={`w-10 h-10 flex items-center justify-center border-gray-300 rounded ${
                    currentPage === index + 1 
            ...
            <a href="#job-list">
              <img src={right_arrow_icon} alt="Next page" />
            </a>
          </div>
        )}
      </section>
